...
def dump_game(
    ...
    *,
    ...
    enrichment_outputs = dump_enrichment.write_dump_enrichment(
        output_dir,
        game_version=versions.game_version,
        unity_version=versions.unity_version,
    )
    print("\nDump enrichment complete:")
    for label, path in enrichment_outputs.items():
        print(f"  {label}: {path}")
    return versions

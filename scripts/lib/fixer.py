...
def _find_similar(target: str, candidates: list[str], max_distance: int = 4) -> list[str]:
...
def _semantic_member_hint(dc: DumpClass, ref_type: RefType, candidates: list[str]) -> str:
    """Return type-oriented guidance for similar member names."""
    if ref_type != RefType.PROPERTY:
        return ""

    details: list[str] = []
    for candidate in candidates:
        return_type = _property_getter_return_type(dc, candidate)
        if return_type:
            details.append(f"{candidate!r} getter return type '{return_type}'")

    if not details:
        return ""
    return f"{'; '.join(details)}; manual review required before renaming"


def _property_getter_return_type(dc: DumpClass, property_name: str) -> str:
    """Best-effort return type extraction for a property's getter method."""
    getter_name = f"get_{property_name}"
    signatures = dc.methods.get(getter_name, [])
    if not signatures:
        return ""
    decl = signatures[0].split("(", 1)[0].strip()
    tokens = decl.split()
    if len(tokens) < 2:
        return ""
    return tokens[-2]

...
#define EASTL_RING_BUFFER_DEFAULT_NAME                                                                                 \
  EASTL_DEFAULT_NAME_PREFIX " ring_buffer" // Unless the user overrides something, this is "EASTL ring_buffer".
#endif

#define EASTL_MAX_STACK_USAGE 4000
/// iterator_status_flag
///
/// Defines the validity status of an iterator. This is primarily used for iterator validation in debug builds. These
/// are implemented as OR-able flags (as opposed to mutually exclusive values) in order to deal with the nature of
/// iterator status. In particular, an iterator may be valid but not dereferencable, as in the case with an iterator to
/// container end(). An iterator may be valid but also dereferencable, as in the case with an iterator to container
/// begin(). iterator validation in debug builds. These are implemented as OR-able flags (as opposed to mutually
/// exclusive values) in order to deal with the nature of iterator status. In particular, an iterator may be valid but
/// not dereferencable, as in the case with an iterator to container end(). An iterator may be valid but also
/// dereferencable, as in the case with an iterator to container begin().
enum iterator_status_flag {
  ...
  isf_current = 0x02, /// The iterator is valid and points to the same element it did when created. For example, if an
                      /// iterator points to vector::begin() but an element is inserted at the front, the iterator is
                      /// valid but not current. Modification of elements in place do not make iterators non-current.
  isf_can_dereference = 0x04 /// The iterator is dereferencable, which means it is in the range of [begin, end). It may
...
///
template <typename T, typename Pointer, typename Reference, typename Container> struct ring_buffer_iterator {
public:
...
public: // We declare public so that global comparison operators can be implemented without adding an inline level and
        // without tripping up GCC 2.x friend declaration failures. GCC (through at least v4.0) is poor at inlining and
        // performance wins over correctness.
  Container c; // We follow the naming convention established for stack, queue, priority_queue and name this 'c'. This
               // variable must always have a size of at least 1, as even an empty ring_buffer has an unused terminating
               // element.
...
public:
  ...
  ring_buffer(std::initializer_list<value_type> ilist,
              const allocator_type& allocator = std::allocator<T>()); // This function sets the capacity to be equal to
  ...
  void set_capacity(
      size_type n); // Sets the capacity to the given value, including values less than the current capacity. Adjusts
                    // the size downward if n < size, by throwing out the oldest elements in the buffer.
  void reserve(size_type n); // Reserve a given capacity. Doesn't decrease the capacity; it only increases it (for
                             // compatibility with other containers' behavior).
  ...
  */
  ...
  template <typename InputIterator> void insert(const_iterator position, InputIterator first, InputIterator last);
...
ring_buffer_iterator<T, Pointer, Reference, Container>::ring_buffer_iterator(
    ...
    , mContainerIterator(containerIterator)
{
}
...
{
  if (++mContainerIterator == mpContainer->end()) [[unlikely]]
...
ring_buffer_iterator<T, Pointer, Reference, Container>::operator--()
{
  if (mContainerIterator == mpContainer->begin()) [[unlikely]]
...
template <typename T, typename PointerA, typename ReferenceA, typename ContainerA, typename PointerB,
          typename ReferenceB, typename ContainerB>
    ...
    : c() // Default construction with default allocator for the container.
{
  ...
  c.resize(cap + 1); // Possibly we could construct 'c' with size, but c may not have such a ctor, though we rely on it
                     // having a resize function.
  mBegin = c.begin();
  mEnd   = mBegin;
  mSize  = 0;
}
...
ring_buffer<T, Container, Allocator>::ring_buffer(const Container& x)
    : c(x) // This copies elements from x, but unless the user is doing some tricks, the only thing that matters is that
           // c.size() == x.size().
{
  // To do: This code needs to be amended to deal with possible exceptions
  // that could occur during the resize call below.
    ...
    : c(allocator)
{
  ...
  // We add one because the element at mEnd is necessarily unused.
  c.resize(1); // Possibly we could construct 'c' with size, but c may not have such a ctor, though we rely on it having
               // a resize function.
  mBegin = c.begin();
  mEnd   = mBegin;
  mSize  = 0;
    ...
    : c(x.c)
{
  ...
  std::advance(mBegin,
               std::distance(const_cast<this_type&>(x).c.begin(),
    ...
    : c() // Default construction with default allocator for the container.
{
  ...
  swap(x); // We are leaving x in an unusual state by swapping default-initialized members with it, as it won't be
           // usable and can be only destructible.
}
...
ring_buffer<T, Container, Allocator>::operator=(const this_type& x)
{
  if (&x != this) {
    ...
    std::advance(
        mBegin, std::distance(const_cast<this_type&>(x).c.begin(),
                              x.mBegin)); // We can do a simple distance algorithm here, as there will be no wraparound.
    std::advance(mEnd, std::distance(const_cast<this_type&>(x).c.begin(), x.mEnd));
  }
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::this_type& ring_buffer<T, Container, Allocator>::operator=(this_type&& x)
{
  swap(x);
  return *this;
}
...
{
  if (&x != this) {
    const difference_type dBegin =
        std::distance(c.begin(), mBegin); // We can do a simple distance algorithm here, as there will be no wraparound.
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<T, Container, Allocator>::begin() noexcept
{
...
ring_buffer<T, Container, Allocator>::begin() const noexcept
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_iterator
ring_buffer<T, Container, Allocator>::cbegin() const noexcept
{
  return const_iterator(const_cast<Container*>(&c),
                        mBegin); // We trust that the const_iterator will respect const-ness.
}
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<T, Container, Allocator>::end() noexcept
{
  return iterator(&c, mEnd);
}

template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_iterator ring_buffer<T, Container, Allocator>::end() const noexcept
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_iterator
ring_buffer<T, Container, Allocator>::cend() const noexcept
{
  return const_iterator(const_cast<Container*>(&c), mEnd); // We trust that the const_iterator will respect const-ness.
}

template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reverse_iterator ring_buffer<T, Container, Allocator>::rbegin() noexcept
{
...
ring_buffer<T, Container, Allocator>::rbegin() const noexcept
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_reverse_iterator
ring_buffer<T, Container, Allocator>::crbegin() const noexcept
{
  return const_reverse_iterator(const_iterator(const_cast<Container*>(&c), mEnd));
}

template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reverse_iterator ring_buffer<T, Container, Allocator>::rend() noexcept
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_reverse_iterator
ring_buffer<T, Container, Allocator>::rend() const noexcept
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_reverse_iterator
ring_buffer<T, Container, Allocator>::crend() const noexcept
{
  return const_reverse_iterator(const_iterator(const_cast<Container*>(&c), mBegin));
}

template <typename T, typename Container, typename Allocator>
bool ring_buffer<T, Container, Allocator>::empty() const noexcept
{
  return mBegin == mEnd;
}

template <typename T, typename Container, typename Allocator>
bool ring_buffer<T, Container, Allocator>::full() const noexcept
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::size_type ring_buffer<T, Container, Allocator>::size() const noexcept
{
  return mSize;
...
ring_buffer<T, Container, Allocator>::DoGetSize(std::random_access_iterator_tag) const
{
  ...
  //
  const difference_type dBegin = std::distance(const_cast<Container&>(c).begin(), mBegin); // const_cast here solves a
little compiler const difference_type dEnd   = std::distance(const_cast<Container&>(c).begin(), mEnd);   // argument
matching problem.
...
namespace Internal
{
  ...
  // allocator type.
  //
  template <typename T, typename = void> struct has_overflow_allocator : std::false_type {
  };

  template <typename T>
  struct has_overflow_allocator<T, std::void_t<decltype(declval<T>().get_overflow_allocator())>> : std::true_type {
  };
  ...
  template <typename Container> struct GetFixedContainerCtorAllocator<Container, false> {
    auto operator()(Container& c)
    {
...
template <typename Container> struct ContainerTemporary<Container, true> {
...
void ring_buffer<T, Container, Allocator>::resize(size_type n)
{
  ...
  } else // We could do a check here for n != size(), but that would be costly and people don't usually resize things to
         // their same size.
  {
    mEnd = mBegin;
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::size_type ring_buffer<T, Container, Allocator>::capacity() const noexcept
{
  assert(c.size() >= 1); // This is required because even an empty ring_buffer has one unused termination element,
                         // somewhat like a \0 at the end of a C string.
...
void ring_buffer<T, Container, Allocator>::set_capacity(size_type n)
{
  ...
  if (n != capacity) // If we need to change capacity...
  {
    ...
    std::copy(itCopyBegin, end(), cTemp.get().begin()); // The begin-end range may in fact be larger than n, in which
                                                        // case values will be overwritten.
    std::swap(c, cTemp.get());

    mBegin = c.begin();
    mEnd   = mBegin;
    std::advance(mEnd,
                 mSize); // We can do a simple advance algorithm on this because we know that mEnd will not wrap around.
  }
}
...
void ring_buffer<T, Container, Allocator>::reserve(size_type n)
{
  ...
  if (n > (c.size() - 1)) // If we need to grow in capacity... // (c.size() - 1) == capacity(); we are attempting to
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reference ring_buffer<T, Container, Allocator>::front()
{
  return *mBegin;
}

template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_reference ring_buffer<T, Container, Allocator>::front() const
{
  return *mBegin;
}

template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reference ring_buffer<T, Container, Allocator>::back()
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_reference ring_buffer<T, Container, Allocator>::back() const
{
  // return *(end() - 1); // Can't use this because not all iterators support operator-.
...
template <typename T, typename Container, typename Allocator> void ring_buffer<T, Container, Allocator>::pop_back()
{
  assert(mEnd != mBegin); // We assume that size() > 0 and thus that there is something to pop.

  if (mEnd == c.begin()) [[unlikely]]
    mEnd = c.end();
  --mEnd;
  --mSize;
}
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reference ring_buffer<T, Container, Allocator>::push_front()
{
  if (mBegin == c.begin()) [[unlikely]]
    mBegin = c.end();
...
template <typename T, typename Container, typename Allocator> void ring_buffer<T, Container, Allocator>::pop_front()
{
  assert(mBegin != mEnd); // We assume that mEnd > mBegin and thus that there is something to pop.
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reference ring_buffer<T, Container, Allocator>::operator[](size_type n)
{
...
template <typename T, typename Container, typename Allocator> void ring_buffer<T, Container, Allocator>::clear()
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::container_type& ring_buffer<T, Container, Allocator>::get_container()
{
  return c;
}
...
inline bool ring_buffer<T, Container, Allocator>::validate() const
{
  ...
  if (c.empty()) // c must always have a size of at least 1, as even an empty ring_buffer has an unused terminating
                 // element.
    return false;
